...
class HevyGetWorkoutsByExerciseTool extends Tool
{
    ...
    public function __invoke(string $exercise): string
    {
        try {
            ...
            $filteredWorkouts= $workouts->filter(function ($workout) use ($exercise) {
                return collect($workout['exercises'])->contains(function ($exerciseItem) use ($exercise) {
                    return str_contains(strtolower($exerciseItem['title']), strtolower($exercise));
                });
            });
